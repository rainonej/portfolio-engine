...
For a concise comparison of **`src/pages-local`** vs **`src/pages`**, nav warnings, and `/resume` visibility, see **[`route-ownership.md`](route-ownership.md)**. For Calendly-style scheduling URLs and **`SchedulingBlock`**, see **[`scheduling-calendly.md`](scheduling-calendly.md)**. For embedding interactive HTML demos or other iframes inside content pages (including the `.md` / `.mdx` trade-off and the `IframeEmbed` component), see **[`iframe-embeds-and-demos.md`](iframe-embeds-and-demos.md)**.
