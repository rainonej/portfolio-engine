...
for(const id of ['goals','packages','graph','upstream','consumer','audit','roadmap','board','products','gap','epics','sources']){ if(!html.includes(`id="${id}"`)){ ok=false; msgs.push('ERROR: missing section '+id); } }
...
if(epics.length<18){ ok=false; msgs.push('ERROR: expected >=18 epic files, found '+epics.length); } else msgs.push('OK: found '+epics.length+' epic source files.');
if(sections.length<8){ ok=false; msgs.push('ERROR: expected >=8 section files, found '+sections.length); } else msgs.push('OK: found '+sections.length+' section source files.');
