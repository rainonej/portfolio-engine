...
4. **Build Command:** `pnpm --filter "./packages/*" run build && pnpm --filter demo-site build && mkdir -p .vercel && rm -rf .vercel/output && mv examples/demo-site/.vercel/output .vercel/output` — workspace packages must emit `dist/` before Astro loads `astro.config.mjs`. The final `mv` lifts the Build Output API tree that `@astrojs/vercel` hard-codes under the Astro project root (`examples/demo-site/.vercel/output/`) up to `<repo-root>/.vercel/output/`, where Vercel's Build Output API auto-detection actually looks (see [`vercel.json`](../../vercel.json)). The `mkdir -p .vercel` is defensive: Vercel CI happens to create `.vercel/project.json` early, but in clean checkouts that directory may not exist and `mv` would fail without a parent.
